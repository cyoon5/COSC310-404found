...
def recompute_movie_ratings(movie_title: str) -> None:
    ...
    if csv_path.exists():
        with csv_path.open("r", newline="", encoding="utf-8") as csvfile:
            ...
            for row in reader:
                ...
                # Common rating column in our CSV
                rating_val = None
    ...
    if avg_rating is not None:
        ...
        # if no ratings, remove or set to None
        updates["movieIMDbRating"] = None

    updates["totalRatingCount"] = total_ratings_count
    updates["totalUserReviews"] = total_user_reviews

    # Use existing updater to write safely (will serialize dates)
    try:
        update_movies(movie_title, updates)
    except Exception:
        # fallback: write directly
        safe_values = updates
        metadata.update(safe_values)
